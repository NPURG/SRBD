...
filenames2015 = ["E13", "INT"]

filenames2017 = ["genuine_accounts", "fake_followers"]
...
def download_data():
    ...
    for k in range(2):
        ...
        for i in range(len(labels)):
            print("open the file ", filenamesHoney[k])
            max_feature = {}
            min_feature = {}
            mean_feature = {}
            mode_feature = {}
            ...
            for i in range(5):
                with open(basePathHoneypot + readBasePath + filenamesHoney[k] + '/' + labels[i], "r", encoding="utf-8") as f:
                    reader = csv.DictReader(f)
                    for row in reader:
                        if row['id'] not in featrues[i]:
                            featrues[i][row['id']] = [float(row["Syllable Count"]), float(row["Monosyllable Count"]),
                                                      float(row["Polysyllable Count"]), float(row["Lexicon Count"]), float(row["Sentence Count"]),
                                                      float(row["Character Count"]), float(row["Letter Count"]), float(row["The Flesch Reading Ease formula"]),
                                                      float(row["The Flesch-Kincaid Grade Level"]), float(row["The Fog Scale"]), float(row["The SMOG Index"]), float(row["The Coleman-Liau Index"]),
                                                      float(row["Automated Readability Index"]), float(row["Linsear Write Formula"]), float(row["Dale-Chall Readability Score"]),
                                                      float(row["Readability Consensus based upon all the above tests"]), float(row["Spache Readability Formula"]),
                                                      float(row["McAlpine EFLAW Readability Score"]), float(row["Reading Time"]), float(row["Difficult Words"])]
                        else:
                            featrues[i][row['id']].append([float(row["Syllable Count"]),
                                                      ...
                                                      float(row["McAlpine EFLAW Readability Score"]), float(row["Reading Time"],
                                                      float(row["Difficult Words"]))])
        row_labels = ["Syllable Count", "Monosyllable Count", "Polysyllable Count", "Lexicon Count", "Sentence Count",
                     "Character Count", "Letter Count", "The Flesch Reading Ease formula", "The Flesch-Kincaid Grade Level", "The Fog Scale",
                      "The SMOG Index", "The Coleman-Liau Index", "Automated Readability Index", "Linsear Write Formula",
                      "Dale-Chall Readability Score",
                      "Readability Consensus based upon all the above tests", "Spache Readability Formula", "McAlpine EFLAW Readability Score", "Reading Time", "Difficult Words"]
        ...
        for i in range(len(row_labels)):
            for key in max_feature:
                ...
                Featrues[k][key] = [[float(max_feature[key][0]), float(min_feature[key][0]), float(mean_feature[key][0]),
                                  float(median_feature[key][0]), float(mode_feature[key][0])]]
                for i in range(1, len(max_feature[key])):
                    Featrues[k][key].append([float(max_feature[key][i]), float(min_feature[key][i]), float(mean_feature[key][i]), float(median_feature[key][i]), float(mode_feature[key][i])])
        ...
        with open(basePathHoneypot + senmanticBasePath + filenamesHoney[k] + '.csv', "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['id'] in Featrues[k]:
                    Featrues[k][row['id']].append([float(row['result_max']), float(row['result_min']), float(row['result_mean']),
        ...
        with open(basePathHoneypot + sentimentBasePath + filenamesHoney[k] + '.csv', "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['id'] in Featrues[k]:
                    Featrues[k][row['id']].append([float(row['max_neg']), float(row['min_neg']), float(row['mean_neg']), float(row['median_neg']), float(row['zhongshu_neg'])])
                    Featrues[k][row['id']].append([float(row['max_neu']), float(row['min_neu']), float(row['mean_neu']), float(row['median_neu']), float(row['zhongshu_neu'])])
                    Featrues[k][row['id']].append([float(row['max_pos']), float(row['min_pos']), float(row['mean_pos']), float(row['median_pos']), float(row['zhongshu_pos'])])
                    Featrues[k][row['id']].append([float(row['max_compound']), float(row['min_compound']), float(row['mean_compound']), float(row['median_compound']), float(row['zhongshu_compound'])])
        ...
        with open(basePathHoneypot + sentenceBasePath + filenamesHoney[k] + '.csv', "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['id'] in Featrues[k]:
                    Featrues[k][row['id']].append([float(row['TweetSimilarityMax']), float(row['TweetSimilarityMIN']), float(row['TweetSimilarityMEAN']), float(row['TweetSimilarityMEDIAN']), float(row['TweetSimilarityMODE'])])
        ...
        with open(basePathHoneypot + timeGapBasePath + filenamesHoney[k] + '.csv', "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['id'] in Featrues[k]:
                    Featrues[k][row['id']].append([float(row['max_hour']), float(row['min_minute']), float(row['mean']), float(row['median_hour']), float(row['mode_hour'])])
        ...
        with open(basePathHoneypot + urlBasePath + filenamesHoney[k] + '.csv', "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['user_id'] in Featrues[k]:
                    Featrues[k][row['user_id']].append([float(row['proportion']), float(row['proportion']), float(row['proportion']), float(row['proportion']), float(row['proportion'])])
    ...
    for i in range(ll):
        ...
        if(len(Featrues_Positive[ID_positives[list_positive[i]]])==n_feature and len(Featrues_Negative[ID_negatives[list_negative[i]]])==n_feature):
...
def build_modle():
    ...
    conc = tf.reshape(attention, [-1, n_feature*5])
    ...
    print(type(x_train))
    print("batch_size = ", batch_size, ", validation_split = ", validation_split)
    
    
    model.fit(np.array(x_train), np.array(y_train), validation_split=validation_split, epochs=epochs, batch_size=batch_size, verbose=2, callbacks=[early_stopping])
