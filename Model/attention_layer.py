...
def download_data():
    ...
    for k in range(2):
                ...
                with open(basePath2017 + readBasePath + filenames2017[k] + '/' + labels[i], "r", encoding="utf-8") as f:
                    ...
                    for row in reader:
                        if row['id'] not in featrues[i]:
                            featrues[i][row['id']] = [float(row["Syllable Count"]), float(row["Monosyllable Count"]), float(row["Polysyllable Count"]),
                                                      float(row["Lexicon Count"]), float(row["Sentence Count"]), float(row["Character Count"]),
                                                      float(row["Letter Count"]), float(row["The Flesch Reading Ease formula"]), float(row["The Flesch-Kincaid Grade Level"]),
                                                      float(row["The Fog Scale"]), float(row["The SMOG Index"]), float(row["The Coleman-Liau Index"]), float(row["Automated Readability Index"]),
                                                      float(row["Linsear Write Formula"]), float(row["Dale-Chall Readability Score"]), float(row["Readability Consensus based upon all the above tests"]),
                                                      float(row["Spache Readability Formula"]), float(row["McAlpine EFLAW Readability Score"]), float(row["Reading Time"]), float(row["Difficult Words"])
                                                      ]
                        else:
                            featrues[i][row['id']].append([float(row["Syllable Count"]), float(row["Monosyllable Count"]),
                                                      float(row["Polysyllable Count"]),
                                                      float(row["Lexicon Count"]),
                                                      float(row["Sentence Count"]),
                                                      float(row["Character Count"]),
                                                      float(row["Letter Count"]), float(row["The Flesch Reading Ease formula"]),
                                                      float(row["The Flesch-Kincaid Grade Level"]),
                                                      float(row["The Fog Scale"]), float(row["The SMOG Index"]),
                                                      ...
                                                      float(row["Reading Time"]), float(row["Difficult Words"])
                                                      ])
        row_labels = ["Syllable Count", "Monosyllable Count", "Polysyllable Count", "Lexicon Count", "Sentence Count", "Character Count",
                     "Letter Count", "The Flesch Reading Ease formula", "The Flesch-Kincaid Grade Level", "The Fog Scale", "The SMOG Index",
                      "The Coleman-Liau Index", "Automated Readability Index", "Linsear Write Formula", "Dale-Chall Readability Score",
        ...
        with open(basePath2017 + timeGapBasePath + filenames2017[k] + '.csv', "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['id'] in Featrues[k]:
    ...
    data_labels = []
    for i in range(ll):
        if(len(Featrues_Positive[ID_positives[list_positive[i]]])==28 and len(Featrues_Negative[ID_negatives[list_negative[i]]])==28):
...
def build_modle():
    ...
    conc = tf.reshape(attention, [-1, 140])
