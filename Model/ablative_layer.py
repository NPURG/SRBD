...
def download_data():
    ...
    for k in range(2):
                    ...
                    for row in reader:
                        if row['id'] not in featrues[i]:
                            featrues[i][row['id']] = [float(row["Syllable Count"]),
                                                      float(row["Monosyllable Count"]), float(row["Polysyllable Count"]), float(row["Lexicon Count"]),
                                                      float(row["Sentence Count"]),
                                                      float(row["Character Count"]), float(row["Letter Count"]),
                                                      float(row["The Flesch Reading Ease formula"]), float(row["The Flesch-Kincaid Grade Level"]), float(row["The Fog Scale"]),
                                                      float(row["The SMOG Index"]),
                                                      float(row["The Coleman-Liau Index"]), float(row["Automated Readability Index"]),
                                                      float(row["Linsear Write Formula"]),
                                                      float(row["Dale-Chall Readability Score"]), float(row["Readability Consensus based upon all the above tests"]),
                                                      float(row["Spache Readability Formula"]), float(row["McAlpine EFLAW Readability Score"]),
                                                      float(row["Reading Time"]),
                                                      float(row["Difficult Words"])]
                        else:
                            featrues[i][row['id']].append([
                                                      float(row["Syllable Count"]),
                                                      float(row["Monosyllable Count"]), float(row["Polysyllable Count"]),
                                                      float(row["Lexicon Count"]),
                                                      float(row["Sentence Count"]), float(row["Character Count"]),
                                                      float(row["Letter Count"]),
                                                      float(row["The Flesch Reading Ease formula"]),  float(row["The Flesch-Kincaid Grade Level"]),
        ...
        row_labels = ["Syllable Count", "Monosyllable Count", "Polysyllable Count", "Lexicon Count",
                      "Sentence Count", "Character Count", "Letter Count", "The Flesch Reading Ease formula", "The Flesch-Kincaid Grade Level",
                      "The Fog Index", "The SMOG Index", "The Coleman-Liau Index", "Automated Readability Index",
                      "Linsear Write Formula",
                      "Dale-Chall Readability Score", "Readability Consensus based upon all the above tests",
                      "Spache Readability Formula", "McAlpine EFLAW Readability Score", "Reading Time", "Difficult Words"]
